...
function aggregateByMonth(opportunities: Opportunity[]): MonthBucket[] {
  const map = new Map<string, MonthBucket>();

  for (const opp of opportunities) {
    if (!opp.estimatedclosedate) continue;
        ...
        return (
          <g key={bucket.key}>
            ...
            <text
              ...
              fill="#6b7280"
              fontFamily="system-ui, sans-serif"
              transform={`rotate(-45, ${x + barWidth / 2}, ${marginTop + chartHeight + 14})`}
            >
              {bucket.label}
            </text>
          </g>
...
export function OpportunityChart() {
  ...
  const totalCount = opportunities.filter(
    (o) => o.estimatedclosedate && parseValue(o.estimatedvalue) > 0
  ).length;
  const dateRange =
    buckets.length > 0
      ? buckets.length === 1
